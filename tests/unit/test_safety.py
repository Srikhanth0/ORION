...
"""
    ...
    def test_allowed_action_passes(
        ...
        manifest.check(
            "create_issue", {}, category="github"
        )

    def test_denied_action_raises(
        self, manifest: PermissionManifest
    ) -> None:
        """Denied GitHub action raises PermissionDeniedError."""
        with pytest.raises(
            PermissionDeniedError, match="delete_repo"
        ):
            manifest.check(
                "delete_repo", {}, category="github"
            )

    def test_shell_denied_pattern(
        self, manifest: PermissionManifest
    ) -> None:
    ...
    def test_filesystem_denied_path(
        self, manifest: PermissionManifest
    ) -> None:
    ...
    def test_filesystem_allowed_path(
        self, manifest: PermissionManifest, tmp_path: Path
    ) -> None:
    ...
    def test_unknown_category_passes(
        self, manifest: PermissionManifest
    ) -> None:
        """Tool with no matching category rules passes."""
        manifest.check(
            "CUSTOM_TOOL", {}, category="custom"
        )

    def test_not_in_allowed_list_denied(
        self, manifest: PermissionManifest
    ) -> None:
        """GitHub action not in allowed list is denied."""
        with pytest.raises(PermissionDeniedError):
            manifest.check(
                "transfer_repo", {}, category="github"
            )
...
class TestRollbackEngine:
    ...
    def test_checkpoint_creates_point(
    ...
    def test_rollback_file_restore(
    ...
    def test_rollback_file_delete_if_new(
        self, engine: RollbackEngine, tmp_path: Path
    ) -> None:
    ...
    def test_rollback_irreversible_warning(
        self, engine: RollbackEngine
    ) -> None:
    ...
    def test_no_checkpoints_rollback(
        self, engine: RollbackEngine
    ) -> None:
...
class TestDestructiveOpGate:
    ...
    async def test_auto_high_risk_no_hitl_approved(
        ...
        gate = DestructiveOpGate(mode="auto")
        result = await gate.approve(
            "delete_repo", {"repo": "org/production"}
        )
    ...
    async def test_strict_with_hitl_approval(self) -> None:
        ...
        gate = DestructiveOpGate(
            mode="strict", hitl_gateway=MockHITL()
        )
